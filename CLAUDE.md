...
- **`src/api/client.ts`** (`ApiClient`) e **`src/contexts/AuthContext.tsx`** — infraestrutura HTTP/auth mais nova, ainda pouco adotada (poucos componentes migrados). O padrão dominante no frontend continua sendo `authFetch` (ver abaixo); não assumir que `ApiClient` é o padrão vigente sem checar o componente específico.
