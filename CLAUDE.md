...
- **`server/email.ts`** — envio de e-mail transacional via `nodemailer` com SMTP genérico (`SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`/`EMAIL_FROM`), não amarrado a um provedor específico. Usado hoje só pelo link de "esqueci minha senha" (`POST /api/auth/forgot-password` em `server.ts`); se `SMTP_HOST` não estiver configurado, a rota informa que a recuperação automática está indisponível em vez de falhar.
