...
Variáveis de ambiente (ver `.env.example`): `GEMINI_API_KEY` (obrigatória para chamadas à Gemini API), `APP_URL`, `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` (obrigatórias — Postgres, Auth e Storage), `JWT_SECRET` (obrigatória — assina os tokens de sessão), `ALLOWED_ORIGINS` (CORS), `ENABLE_AVATAR_AI`. Ver `.DEPLOY.md` para o guia completo de setup do Supabase e deploy no Render.
...
- **`server/statsEngine.ts`** — cálculo de estatísticas por temporada (`computeStatsForSeason`), incluindo streaks — sempre recalculados dinamicamente a partir das partidas filtradas da temporada (nunca usar `player.currentStreak` global diretamente para views filtradas por temporada; ver histórico do bug em `AUDITORIA.md` §"Bug de Estatísticas de Streaks").
- **`server/avatarProvider.ts`** — fábrica de provedores de geração de avatar/card via Gemini (`AvatarProviderFactory`).
...
- **Uploads** (fotos de jogador, mídia do mural) — Supabase Storage (bucket público `uploads`), com validação de tipo por magic bytes (`file-type`) e tamanho real do buffer decodificado. Não há mais disco local (`data/uploads/`) nem simulação de S3.
...
- **Sorteio de times**: fluxo `/api/matches/:matchId/draw` → `/api/draws/:drawId/update-manual` (ajuste manual) → `/api/draws/:drawId/confirm-lock` (trava o sorteio). Atenção: `confirm-lock` e `/results` podem ambos incrementar afinidades de duplas/trios se acionados em sequência na mesma semana — ver `AUDITORIA.md` para o problema conhecido de duplicidade.

Para entendimento aprofundado de regras de negócio, edge cases já mapeados e problemas conhecidos, consulte `AUDITORIA.md` (auditoria funcional completa) e `docs/` (arquitetura da home dinâmica e handoff).
